...
public:
	...
	void add_frame(IMAGE* image, int x_num, int y_num) {
		int width = image->getwidth();
		int height = image->getheight();
		int width_frame = width / x_num;
		int height_frame = height / y_num;
		for (int j = 0; j < y_num; j++) {
			for (int i = 0; i < x_num; i++) {
				Rect rect_src;
				rect_src.x = i * width_frame, rect_src.y = j * height_frame;
	...
	void on_render() {//‰÷»æ
		std::cout << "frame_list size: " << frame_list.size() << ", idx_frame: " << idx_frame << std::endl;
