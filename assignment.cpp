		...
		if (is_blood3_anim_playing) {
			...
			if (delta_time >= 1000) {
				...
				if (blood3_x_left <= window_width && blood3_x_right >= -75)
				{
					...
					blood3->play(blood3_x_right, blood3_y_start - 50, 20, 2);
	...
	void Bullet1(Player& paimon) {
		...
		if (is_bullet_anim_playing) {
			...
			if (bullet_direction == 1) {
				...
				if (bullet_x <= bullet_x_start - 150) {
					bullet->play(bullet_x +100 , bullet_y_start, 45, 2);
				}
			}
...
public:
	Boss3(LPCTSTR path, int width, int height, int sequence) : Boss(path, width, height, sequence){
	}
	~Boss3() override {
		delete pull;
		delete explode;
...
public:
	...
	int frame_width = 150;
	int frame_height = 131;

public:
	Ghost(LPCTSTR path, int width, int height, int sequence) : Boss(path, width, height, sequence){
		ghost = new Animation_new(_T("boss3/ghost1.png"), 4, 5, 150,131,45, 0);
	}
	~Ghost() {
		delete ghost;
	}
	void drawghost(Player& paimon, Boss3* boss3) {
		//boss上方小血条
		if (!visited_position) {
			if (paimon.GetPosition().x > boss3->position.x && paimon.GetPosition().y<=boss3->position.y) {
				position.x = max(2 * paimon.GetPosition().x - boss3->position.x, window_width - 200);
				position.y = min(2*paimon.GetPosition().y - boss3->position.y, 50);
			}
			else if (paimon.GetPosition().x > boss3->position.x && paimon.GetPosition().y >= boss3->position.y) {
				position.x = max(2 * paimon.GetPosition().x - boss3->position.x, window_width - 200);
				position.y=max(2 * paimon.GetPosition().y - boss3->position.y, window_height-200);
			}
			else if (paimon.GetPosition().x <= boss3->position.x && paimon.GetPosition().y <= boss3->position.y) {
				position.x = min(2 * paimon.GetPosition().x - boss3->position.x, 50);
				position.y = min(2 * paimon.GetPosition().y - boss3->position.y, 50);
			}
			else if (paimon.GetPosition().x <= boss3->position.x && paimon.GetPosition().y >= boss3->position.y) {
				position.x = min(2 * paimon.GetPosition().x - boss3->position.x, 50);
			...
			direction = position.x > paimon.GetPosition().x ? 1 : 2;
		...
		if (!visited_generate) {
			...
			visited_generate = true;
		}

		setlinecolor(RGB(0, 0, 0));
		rectangle(position.x + 0.5 * (frame_width - 0.6*max_hp), position.y - 5, position.x + 0.5 * (frame_width + 0.6 * max_hp), position.y - 2);

		if (current_hp < max_hp) {//损失血条
			setfillcolor(RGB(255, 193, 193));
			fillrectangle(position.x + 0.5 * (frame_width - 0.6 * max_hp) + 0.6 * current_hp, position.y - 5, position.x + 0.3 * (frame_width + 0.6 * max_hp), position.y - 2);
		}

		//实时血条
		setfillcolor(RGB(238, 44, 44));
		fillrectangle(position.x + 0.5 * (frame_width - 0.6 * max_hp), position.y - 5, position.x + 0.5 * (frame_width - 0.6 * max_hp) + 0.6 * current_hp, position.y - 2);
	}
};
...
void generateghost(vector<Ghost*>& ghost_list,Ghost& ghost){
...
int main() {
	...
	Boss3* boss3 = new Boss3(_T("boss%d/boss%d.png"), 175, 221, 3);
	Ghost* ghost = new Ghost(_T("boss%d/boss%d.png"), 175, 221, 3);
