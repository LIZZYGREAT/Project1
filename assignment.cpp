...
public:
	...
	void draw(int delta) {
		...
		if (is_fireflush_anim_playing) {
			...
			int fireflush_x = fireflush_start_x + 0.5 * frame_width + elapsed_time * 0.5 * fireflush_direction;
			int fireflush_y = fireflush_start_y + 0.2 * frame_height;
...
public:
	...
	bool checkexplodecollision(Player& paimon) {
		const POINT& paimon_position = paimon.GetPosition();
	...
	bool checkfireballcollision(Player& paimon) {
		...
		return distance <= fireball_radius;
	}
	...
	while (running) {
		...
		if (is_game_started)
		{
			...
			for (Enemy* boar : boar_list) {
				...
				if (boar->checkfireballcollision(paimon)) {
				}
				if (boar->checkexplodecollision(paimon)) {
				}
