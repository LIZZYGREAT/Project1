...
public:
	...
	void skill2_fireflush() {  //技能3	 //耗蓝50  10S	 伤害35点 只可向左右方向进行发射
	...
	void checkexplodecollision(Player& paimon) {
		...
		if (paimon.is_explode_anim_playing) {
			...
			if (distance <=  0.5*explosion_radius &&current_time-lastexplodehit>=1000) {
					...
					current_hp_player += min(3, paimon.max_length - current_hp_player);
...
public:
	...
	void generateSafe(Player& paimon) {
		...
		if (stage == 2) {
			std::uniform_int_distribution<> radiusDis(200, 599);
			safe_radius_max = radiusDis(gen);
			safe_radius_min = 0.5 * safe_radius_max;
		}
		if (stage == 3) {
			std::uniform_int_distribution<> radiusDis(200, 549);
			safe_radius_max = radiusDis(gen);
			safe_radius_min = 0.5 * safe_radius_max;
		}
	}
	...
	void randomMove(Player& paimon) {
		...
		if (playerDirLen != 0) {
			...
			cout <<"融合前" << current_dir_x << " " << current_dir_y << endl;
			...
			cout <<"融合后"<< current_dir_x << " " << current_dir_y << endl;
		...
		else if (new_x + frame_width >= window_width-1.3*frame_width) {
		...
		else if (new_y + frame_height >= window_height-0.5*frame_height) {
		...
		DWORD currentTime = GetTickCount();
		if (currentTime - lastFacingChangeTime >= facingChangeInterval) {
			if (current_dir_x < 0) {
				facing_left = true;
				lastFacingChangeTime = currentTime;
			}
			else if (current_dir_x > 0) {
				facing_left = false;
				lastFacingChangeTime = currentTime;
			}
		}
	...
	void move(Player& paimon) {
		DWORD currentTime = GetTickCount();
		if (currentTime - lastSafetyCheckTime >= 2000) { // 每 8 秒检查一次安全范围
			lastSafetyCheckTime = currentTime;
			if (!isInSafe(paimon)) {
				generateSafe(paimon); // 重新生成安全范围
				std::cout << "Regenerated safe range. Center: (" << safecenter_x << ", " << safecenter_y << "), Radius: [" << safe_radius_min << ", " << safe_radius_max << "]" << std::endl;
			}
		}
		if (isInSafe(paimon)) {
			std::cout << "Boss is in safe range. Moving randomly." << std::endl;
			std::cout << "Speed: " << speed << ", Direction: (" << current_dir_x << ", " << current_dir_y << ")" << std::endl;
			randomMove(paimon); // 在安全范围内随机移动
		}
		else {
			std::cout << "Boss is out of safe range. Moving to safe range." << std::endl;
			std::cout << "Speed: " << speed << ", Direction: (" << current_dir_x << ", " << current_dir_y << ")" << std::endl;
			moveToSafe(); // 不在安全范围内，移动到安全范围
		}
		std::cout << "Boss position: (" << position.x << ", " << position.y << ")" << std::endl;
	}
	...
	void draw(int delta) {
		...
		//屏幕上方大血条
		setlinecolor(RGB(0, 0, 0));
		rectangle(0.4 * (window_width - max_hp), 90, 0.4 * (window_width + max_hp), 100);

		if (current_hp < max_hp) {//损失血条
			setfillcolor(RGB(255, 193, 193));
			fillrectangle(0.4 * (window_width - max_hp) + 0.8*current_hp, 90, 0.4 * (window_width + max_hp), 100);
		}

		//实时血条
		setfillcolor(RGB(238, 44, 44));
		fillrectangle(0.4 * (window_width - max_hp), 90, 0.4 * (window_width - max_hp) + 0.8*current_hp, 100);
...
int main() {
	...
	Player paimon;
	Boss boss1(_T("boss%d/boss%d.png"),170,188,1);
