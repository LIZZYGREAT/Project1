...
class Animation_new {
public:
	Animation_new(LPCTSTR path, int x, int y, int width, int height, int interval,int z){  //参数讲解： x为横向个数，y为纵向个数，width，height分别为单个的宽高，interval为帧率间隔，z是判断是否需要处理黑色背景
		interval_ms = interval;                                                   
		for (int i = 0; i < y; ++i) {
			for (int j = 0; j < x; ++j) {
				IMAGE * frame = new IMAGE();
...
public://处理受伤问题
	...
	DWORD boss2_lastblood2hit = 0;
	DWORD boss2_lastblood2hit2 = 0;
	...
	void pocessevent(const ExMessage& msg) {
		if (msg.message == WM_KEYDOWN) {
			...
			case '2':
				...
				break;
			case VK_SPACE:
				if (GetTickCount() - is_shift_end > 6000) {  // 检查冷却时间
					is_shift = true;
					skillspace_shift();
			...
			case 'Q':
				...
				break;
			case 'R':
				if (GetTickCount() - is_explode_end > 8000) {
					is_explode = true;
					skillr_expolde();
		...
		if (is_fireflush_anim_playing) {
			...
			if (!fireflush_animation_ended) {
				if (elapsed_time < fireflush->getTotalDuration() &&(fireflush_x  <= window_width) &&(fireflush_x  >=0 )) {
					fireflush->play(fireflush_x, fireflush_y, 30, fireflush_direction == 1 ? 1 : 2);
				}
				else {
					// 动画第一轮播放结束，设置标志位
			...
			if (fireflush_animation_ended) {
				...
				if((fireflush_x  <= window_width) &&(fireflush_x  >= 0)) {
					firepart->play(fireflush_x , fireflush_y, 30, fireflush_direction == 1 ? 1 : 2);
				}
				else {
...
public:
	Boss(LPCTSTR path, int width, int height, int sequence) {
		TCHAR path_file[256];
		// 修正路径处理逻辑
		_stprintf_s(path_file, path, sequence,sequence);
		boss = new Animation_new(path_file, 4, 2, width, height, 45, 0);

		blood2 = new Animation_new(_T("boss1/blood2.png"), 5, 1, 150, 150, 45, 1);
	...
	void Blood2(Player& paimon) {
		...
		if (is_blood2_anim_playing) {
			...
			double distance = sqrt(dx * dx + dy * dy);
			if (delta_time < 1000) {
				blood2->play(position.x + 0.5 * frame_width-75, position.y + 0.5 * frame_height-75, 45, 1);
	...
	void draw(int delta) {
		//boss上方小血条
		setlinecolor(RGB(0, 0, 0));
		rectangle(position.x + 0.5 * (frame_width - 0.1 * max_hp), position.y - 5, position.x + 0.5 * (frame_width + 0.1 * max_hp), position.y - 2);

		if (current_hp < max_hp) {//损失血条
			setfillcolor(RGB(255, 193, 193));
			fillrectangle(position.x + 0.5 * (frame_width - 0.1 * max_hp) + 0.1 * current_hp, position.y - 5, position.x + 0.5 * (frame_width + 0.1 * max_hp), position.y - 2);
		}

		//实时血条
		setfillcolor(RGB(238, 44, 44));
		fillrectangle(position.x + 0.5 * (frame_width - 0.1 * max_hp), position.y - 5, position.x + 0.5 * (frame_width - 0.1 * max_hp) + 0.1 * current_hp, position.y - 2);

		//屏幕上方大血条
		setlinecolor(RGB(0, 0, 0));
		rectangle(0.5 * (window_width - 0.8*max_hp), 90, 0.5 * (window_width + 0.8*max_hp), 100);

		if (current_hp < max_hp) {//损失血条
			setfillcolor(RGB(255, 193, 193));
			fillrectangle(0.5 * (window_width - 0.8*max_hp) + 0.8*current_hp, 90, 0.5 * (window_width +0.8*max_hp), 100);
		}

		//实时血条
		setfillcolor(RGB(238, 44, 44));
		fillrectangle(0.5 * (window_width - 0.8*max_hp), 90, 0.5 * (window_width - 0.8*max_hp) + 0.8*current_hp, 100);
		...
		if (facing_left) {
			...
			boss->play(position.x, position.y, 45, 2);
		}
	}
public:
...
private://处理技能有关cd问题
	//总cd 即技能间隔不必过密希望boss间隔2s+rand（）%1000再释放其他技能
	DWORD skillReleaseTime = 0;
	DWORD skill_interval = 2500+rand()%500;
...
public:
	Boss1(LPCTSTR path, int width, int height, int sequence) : Boss(path, width, height, sequence) {
		blood1 = new Animation_new(_T("boss1/blood1.png"), 3, 1,150 ,150, 200, 1);
		blood2 = new Animation_new(_T("boss1/blood2.png"), 5, 1,150 ,150, 45, 1);
		blood3 = new Animation_new(_T("boss1/blood3.png"), 3, 1, 150, 150, 200, 1);
		bullet = new Animation_new(_T("boss1/bullet.png"), 5, 1, 150, 150, 45, 1);
	}
	...
	void Blood1(Player& paimon) {
		DWORD current_time = GetTickCount();
		if (current_time - is_Blood1_end > 5000+rand()%3000 && current_time-skillReleaseTime>=skill_interval) {
			is_blood1_anim_playing = true;
			skillReleaseTime = GetTickCount();
		}
		if (is_blood1_anim_playing) {
			...
			if (blood1_direction==1) {
				...
				blood1->play(blood1_x, blood1_y_start, 20, 2);
			}
	...
	void Blood3(Player &paimon) {
		DWORD current_time = GetTickCount();
		if (current_time - is_Blood3_end > 10000 + rand() %2000 && current_time-skillReleaseTime>=skill_interval) {
		...
		if (is_blood3_anim_playing) {
			...
			int blood3_x_left = blood3_x_start_left + 0.9 * (delta_time - 1000);
			int blood3_x_right = blood3_x_start_right - 0.9 * (delta_time - 1000);
			cout << delta_time << endl;
			cout << blood3_x_left << endl;
			cout << blood3_x_right << endl;
			if (delta_time < 1000) {
			...
			if (delta_time >= 1000) {
				...
				{
					blood3->play(blood3_x_left, blood3_y_start-50, 30, 1);
					blood3->play(blood3_x_right, blood3_y_start-50, 30, 2);
	...
	void Bullet(Player &paimon) {
		DWORD current_time = GetTickCount();
		if (current_time - is_Bullet_end > 1000 + rand() % 1000 && current_time - skillReleaseTime >= skill_interval) {
			is_bullet_anim_playing = true;
			skillReleaseTime = GetTickCount();
		}
		if (is_bullet_anim_playing) {
			if (!visited_bullet && position.y + 0.5 * frame_height + 200 >= paimon.GetPosition().y + 0.5 * paimon.frame_height && position.y + 0.5 * frame_height - 200 <= paimon.GetPosition().y + 0.5 * paimon.frame_height) {
			...
			if (is_overlap_x && is_overlap_y && GetTickCount() - paimon.lastbullethit >= 1000) {
				if (protectivevalue >= 25) {
					protectivevalue -= 25;
				}
				else {
					current_hp_player -= 25 - protectivevalue;
					protectivevalue = 0;
				}
				paimon.lastbullethit = current_time;
			}
			if (bullet_x + 76 <= 0 || bullet_x >= window_width) {
	...
	void RandomlyReleaseSkill(Player& paimon) {
		...
		if (IsAnySkillPlaying()) {
			return; // 有技能正在释放，不进行新技能选择
		}

		DWORD current_time = GetTickCount();
		if (current_time - skillReleaseTime < skill_interval) {
			return; // 未达到技能释放间隔，不释放技能
		}
	}
};
...
private:
	...
	bool IsAnySkillPlaying() const {
		return is_redexplode_anim_playing || is_blood2_anim_playing;
	}
	int skillChoice = 0;
...
public:
	...
	Boss2(LPCTSTR path, int width, int height, int sequence) : Boss(path, width, height, sequence) {
		frame_width = width;
		frame_height = height;
		pull = new Animation_new(_T("boss2/pull.png"),5, 8, 200, 200,45,1);
		redexplode = new Animation_new(_T("boss2/redexplode.png"), 5, 7, 200, 200, 45, 1);
		redeye = new Animation_new(_T("boss2/redeye.png"), 5, 8, 200, 200, 45, 1);
		blood2 = new Animation_new(_T("boss1/blood2.png"), 5, 1, 150, 150, 45, 1);
	}
	~Boss2() override {
		delete pull;
	...
	void Redexplode(Player& paimon) {
		DWORD current_time = GetTickCount();
		if (current_time - is_redexplode_end > 5000 + rand() % 4000 && current_time - skillReleaseTime >= skill_interval) {
			is_redexplode_anim_playing = true;
			skillReleaseTime = GetTickCount();
		}
		if (is_redexplode_anim_playing) {
...
int main() {
	...
	Player paimon;
	Boss1* boss1=new Boss1 (_T("boss%d/boss%d.png"),170,188,1);
	Boss2* boss2 = new Boss2(_T("boss%d/boss%d.png" ),180, 162, 2);
