...
void Player::on_update(float delta) {
	...
	if (is_attacking) {
		std::cout << "attack1：" << is_attack1 << std::endl;
		std::cout << "attack2：" << is_attack2 << std::endl;
		std::cout << "attack3：" << is_attack3 << std::endl;
		std::cout << std::endl;
	}
...
void Player::update_hit_box_position() {    //暂时留存，实际应该调整，单独开一个类为玩家攻击碰撞箱
	...
	pos_hit_box.y = player->get_logic_center().y;
	pos_hit_box.x = player->get_move_axis() > 0 ? player->get_logic_center().x + 50 : player->get_logic_center().x - 50;
