...
Player::Player() {
	...
	CollisionManager::instance()->get_see_through_box_attack2()->set_on_collide([&]() {
		hurt_box->set_enabled(false);
		is_seeThrough_visible = true;
		seeThrough_invulnerable.restart();
		});


	CollisionManager::instance()->get_see_through_box_attack3()->set_on_collide([&]() {
		hurt_box->set_enabled(false);
		is_seeThrough_visible = true;
		seeThrough_invulnerable.restart();
		});
	...
	seeThrough_invulnerable.set_wait_time(1.0f);
	seeThrough_invulnerable.set_one_shot(true);
	seeThrough_invulnerable.set_on_timeout([&]() {
		hurt_box->set_enabled(true);
		is_seeThrough_visible = false;
		});
...

	{
		...
		animation_seeThrough1.set_interval(0.08f);
		animation_seeThrough1.set_loop(true);
		animation_seeThrough1.set_anchor_mode(Animation::AnchorMode::Centered);
		animation_seeThrough1.add_frame(ResourcesManager::instance()->find_image("see_through1"), 3, 3);
		animation_seeThrough1.set_on_finished([&]() {
			is_seeThrough_visible = false;
			});

		animation_seeThrough2.set_interval(0.08f);
		animation_seeThrough2.set_loop(true);
		animation_seeThrough2.set_anchor_mode(Animation::AnchorMode::Centered);
		animation_seeThrough2.add_frame(ResourcesManager::instance()->find_image("see_through2"), 3, 3);
		animation_seeThrough2.set_on_finished([&]() {
			is_seeThrough_visible = false;
			});
...
void Player::on_update(float delta) {
	...
	if (!is_attacking) {
		is_animation_facing_left = is_facing_left;
	}
	...
	if (is_seeThrough_visible)
	{
		animation_seeThrough1.set_position(get_logic_center() + vector2({ 0,20 }));
		animation_seeThrough1.on_update(delta);
		animation_seeThrough2.set_position(get_logic_center()+ vector2({ 0,20 }));
		animation_seeThrough2.on_update(delta);
		seeThrough_invulnerable.on_update(delta);
	}
...
void Player::on_render() {
	...
	if (is_seeThrough_visible) {
		animation_seeThrough1.on_render();
		animation_seeThrough2.on_render();
	}
...
void Player::draw_hp() {
	...
	int hp_bar_width = (end_x - start_x) * max(0, hp) / max_hp;
